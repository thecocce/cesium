   ...
   function update_selection(obj)
   {
    ...
    var SelectionWidget = function(container, canvas, camera, ellipsoid, options) {
        ...
        this._div.setAttribute("id", "SelectionWidget");
    ...
    SelectionWidget.prototype.update = function() {
        ...
        if (!Cesium.defined(this.target) || this.target.ready==false)
        {
            return;
        }
        ...
        var isBillboard = Cesium.defined(this.target.pixelOffset); // test if billboard (change this later)
        ...
        {
            var sphere = this.target.boundingSphere;
