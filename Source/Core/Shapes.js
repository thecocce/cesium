...
define([
    ...
    var unitCartScratch = new Cartesian3();
    var rotAxisScratch = new Cartesian3();
    ...
    function _computeEllipseQuadrant(cb, cbRadius, aSqr, bSqr, ab, ecc, unitPos, eastVec, northVec, rotation,
        ...
        for (var i = 0; i < numPts; i++, thetaPtsIndex += clockDir, ++ellipsePtsIndex) {
            ...
            rotAxis = Cartesian3.multiplyByScalar(eastVec, temp, rotAxisScratch);

            temp = Math.sin(azimuth);
            tempVec = Cartesian3.multiplyByScalar(northVec, temp, tempVecScratch);
            ...
            var unitCart = Cartesian3.normalize(tmpEllipsePts, unitCartScratch);
            tmpEllipsePts = Cartesian3.multiplyByScalar(unitCart, cbRadius, tmpEllipsePts);
     ...
     */
    Shapes.computeCircleBoundary = function(ellipsoid, center, radius, granularity) {
    ...
    var surfPosScratch = new Cartesian3();
    var unitPosScratch = new Cartesian3();
    var tempVecScratch2 = new Cartesian3(0.0, 0.0, 1);
     ...
     */
    Shapes.computeEllipseBoundary = function(ellipsoid, center, semiMajorAxis, semiMinorAxis, rotation, granularity) {
        ...
        var surfPos = Cartesian3.clone(center, surfPosScratch);
        var surfPosMag = Cartesian3.magnitude(surfPos);


        var temp = 1.0 / surfPosMag;

        var unitPos = Cartesian3.multiplyByScalar(surfPos, temp, unitPosScratch);
        var eastVec = Cartesian3.normalize(Cartesian3.cross(tempVecScratch2, surfPos, eastVecScratch), eastVecScratch);
        ...
        ellipsePts.push(Cartesian3.clone(ellipsePts[0], new Cartesian3())); // Duplicates first and last point for polyline
     ...
     */
    Shapes.compute2DCircle = function(radius, granularity) {
