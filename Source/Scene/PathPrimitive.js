...
define([
    ...
    // p is a Cartographic point
    function getUpVector(p, ellipsoid)
    {
        var temp = new Cartographic(p.longitude, p.latitude, p.height + 100.0);
    ...
    var PathPrimitive = function(points, ellipsoid, options) {
        ...
        if (!defined(animated_path_material)) {
            animated_path_material = new Material({
            fabric : {
                type : 'AnimatedImage',
                uniforms : {
        ...
        this._radius = defaultValue(options.width, 50);
        this._speed = defaultValue(options.speed, 30);
